...
def validate_operation_ids(operations: List[Operation]) -> List[str]:
    ...
    null_ids = [i for i, op in enumerate(operations) if op.op_id is None or op.op_id == ""]
    if null_ids:
    ...
    if min_id != 1:
        errors.append(f"Operation IDs must start from 1, found minimum ID: {min_id}")
...
def validate_predecessors(operations: List[Operation]) -> List[str]:
    ...
    for op in operations:
        ...
        if len(op.predecessors) != len(set(op.predecessors)):
            duplicates = [pid for pid in op.predecessors if op.predecessors.count(pid) > 1]
            errors.append(f"Operation {op.op_id} has duplicate predecessors: {duplicates}")
        
        # Check for self-references
        if op.op_id in op.predecessors:
            errors.append(f"Operation {op.op_id} cannot depend on itself (self-reference)")
        
        # Check each predecessor
        for pred_id in op.predecessors:
            # Check if referenced ID exists
            if pred_id not in valid_ids:
                errors.append(f"Operation {op.op_id} references non-existent predecessor ID: {pred_id}")
            ...
            if pred_id in used_predecessor_ids:
                ...
                for other_op in operations:
                    if pred_id in other_op.predecessors and other_op.op_id != op.op_id:
                        errors.append(f"Operation ID {pred_id} is already used as predecessor for operation {other_op.op_id}, cannot be used again for operation {op.op_id}")
                        break
            else:
    ...
    if max_id in used_predecessor_ids:
        referencing_ops = [op.op_id for op in operations if max_id in op.predecessors]
        errors.append(f"Highest operation ID ({max_id}) should not be referenced as a predecessor (referenced by: {referencing_ops})")
    ...
    expected_ids = set(range(min_id + 1, max_id))  # All IDs except first and last
    for expected_id in expected_ids:
        count = predecessor_counts.get(expected_id, 0)
        if count == 0:
            errors.append(f"Operation ID {expected_id} is missing from predecessor column (should appear exactly once)")
        elif count > 1:
            errors.append(f"Operation ID {expected_id} appears {count} times in predecessor column (should appear exactly once)")
...
def read_operations(filepath: str) -> List[Operation]:
    ...
    if path.suffix.lower() in (".xlsx", ".xls"):
        ...
        df = pd.read_csv(path, dtype=str)  # Read all columns as strings to prevent auto-conversion
    ...
    if not pred_col:
        raise ValueError(f"Missing required column: Predecessor (or Predecessor/Pred/Depends On/Previous)")
    ...
    for row_idx, row in df.iterrows():
        ...
        if op_id_col:
            ...
            if not row[op_id_col] or str(row[op_id_col]).strip().lower() in ['nan', 'none', '']:
                raise ValueError(f"Row {row_idx + 1}: Operation ID is missing/null")
            
            # Check if the ID value contains commas (multiple IDs in one cell)
            id_str = str(row[op_id_col]).strip()
            if ',' in id_str:
                raise ValueError(f"Row {row_idx + 1}: Operation ID cannot contain multiple values (found '{id_str}')")
            
            try:
                # First try to convert to float (handles both int and float strings), then to int
                op_id = int(float(row[op_id_col]))
                if op_id <= 0:
                    raise ValueError(f"Row {row_idx + 1}: Operation ID must be positive, got {op_id}")
            except (ValueError, TypeError) as e:
                raise ValueError(f"Row {row_idx + 1}: Invalid operation ID '{row[op_id_col]}' - {str(e)}")
        else:
            # Use row position as ID (1-indexed)
            op_id = row_idx + 1
        
        # Extract operation name
        name = str(row[name_col]).strip() if row[name_col] and str(row[name_col]).strip().lower() not in ['nan', 'none', ''] else ""
        
        # Extract machine type
        machine_type = str(row[machine_col]).strip() if row[machine_col] and str(row[machine_col]).strip().lower() not in ['nan', 'none', ''] else ""
        ...
        if pred_col:
            ...
            if not pred_value or str(pred_value).strip().lower() in ['nan', 'none', '']:
                raise ValueError(f"Row {row_idx + 1}: Predecessor cell is empty/null")
            try:
